...
async function loadUploadsList() {
  ...
  if (!uploads.length) { el.innerHTML = '<p class="muted">No uploads yet.</p>'; return; }
  el.innerHTML = `<table><thead><tr><th>Title</th><th>Type</th><th>Date</th><th></th></tr></thead><tbody>
    ${uploads.map(u => `<tr><td>${escapeHtml(u.title)}</td><td>${u.content_type}</td><td class="muted">${formatDate(u.created_at)}</td>
    <td><button class="btn btn-danger" data-delete="${u.id}">Remove</button></td></tr>`).join('')}
  </tbody></table>`;
...
async function loadUsers() {
  ...
  document.getElementById('users-table').innerHTML = users.map(u => `
    <tr>
      ...
      <td><button data-toggle="${u.id}" data-active="${u.is_active}" ${u.id===currentUser.id?'disabled':''}>${u.is_active?'Active':'Inactive'}</button></td>
      <td>${u.id !== currentUser.id ? `<button data-del="${u.id}" class="btn btn-danger">Delete</button>` : ''}</td>
    </tr>`).join('');
...
async function loadTeachers() {
  ...
  document.getElementById('teachers-list').innerHTML = teachers.map(t => `
    <div style="padding:0.75rem 0;border-bottom:1px solid #eee">
      <strong>${escapeHtml(t.full_name)}</strong> · ${escapeHtml(t.email)}
      <p class="muted" style="margin:0.25rem 0 0">${t.assignments?.map(a => `${a.subject} (${a.grade} ${a.section})`).join(' · ') || 'No assignments'}</p>
    </div>`).join('') || '<p class="muted">No teachers</p>';
}
...
async function loadStudents(grade = '', section = '') {
  ...
  document.getElementById('students-table').innerHTML = students.map(s => `
    ...
    </tr>`).join('') || '<tr><td colspan="5" class="muted">No students</td></tr>';
}
...
async function loadMasterGradebook() {
  ...
  document.getElementById('gradebook-table').innerHTML = grades.map(g => `
    ...
    </tr>`).join('') || '<tr><td colspan="6" class="muted">No grades</td></tr>';
}
...
async function loadFilters() {
  ...
  ['filter-grade', 'gb-grade'].forEach(id => {
    ...
    el.innerHTML = '<option value="">All grades</option>' + grades.map(g => `<option>${g}</option>`).join('');
  });
  ['filter-section', 'gb-section'].forEach(id => {
    const el = document.getElementById(id);
    el.innerHTML = '<option value="">All sections</option>' + sections.map(s => `<option>${s}</option>`).join('');
  });
}
