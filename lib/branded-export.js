...
function groupGradebookEntries(rows) {
  const groups = new Map();

  rows.forEach(row => {
    const grade = row.grade_level || row.grade || '';
    const section = row.section || '';
    const key = `${grade}|${section}`;
    const sheetName = `${grade} ${section}`.trim() || 'Gradebook';
    const title = `${sheetName} — Gradebook`;

    if (!groups.has(key)) {
      groups.set(key, { sheetName, title, subtitle: SCHOOL_SUBTITLE, rows: [] });
    }
...
function addBrandedDataSheet(workbook, { sheetName, title, subtitle, columns, rows }) {
  const sheet = workbook.addWorksheet(safeSheetName(sheetName), {
    views: [{ showGridLines: false }]
  });
  ...
  addBrandedHeader(sheet, { title, totalColumns: columns.length, extraLine: subtitle || SCHOOL_SUBTITLE });
  addLogo(sheet, workbook);
  ...
  rows.forEach((row, rowOffset) => {
    ...
    columns.forEach((col, colIndex) => {
      ...
      styleBodyCell(cell, { shaded: rowOffset % 2 === 1 });
    });
    excelRow.height = 20;
  });
...
async function buildGradebookWorkbook({ sheets }) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = SCHOOL_NAME;
  workbook.created = new Date();

  sheets.forEach(({ sheetName, title, subtitle, columns, rows }) => {
    addBrandedDataSheet(workbook, { sheetName, title, subtitle, columns, rows });
  });

  return workbook;
}

async function buildGradebookBuffer(options) {
  const workbook = await buildGradebookWorkbook(options);
  return workbook.xlsx.writeBuffer();
}

const GRADEBOOK_SHEET_COLUMNS = [
...
async function buildBrandedWorkbook({ title, sheetName, columns, rows, subtitle }) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = SCHOOL_NAME;
  workbook.created = new Date();

  addBrandedDataSheet(workbook, {
